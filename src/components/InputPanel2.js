...
const InputPanel2 = ({ tabIndex, setTabIndex, selectedFiles, setSelectedFiles, inputData }) => {
                ...
                <Box>
                    < Box sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', padding: 2 }}>
                        <TextField
                            ...
                            error={inputData.trim() !== '' && inputDataType !== 'UNKNOWN' && !isInputDataValid}
                            helperText={(inputData.trim() !== '' && inputDataType !== 'UNKNOWN' && !isInputDataValid) ? `The entered data does not conform to the expected ${inputDataType} format.` : ''}
                        />
                    </Box>
